#!/usr/bin/env python3
"""
slack_brief.py - reads the day's web3career_jobs JSON and posts a triaged
daily brief to Slack via an incoming webhook. Run right after the collector.
...
def build(payload: dict, date_str: str):
    ...
    summary = "*%d new signals*  ·  :red_circle: A %d   :large_blue_circle: B %d   :white_circle: C %d\n_Scores are provisional (role + JD only) — company-fit is your call._" % (
        len(sigs), len(a), len(b), len(c))
    ...
    if a:
        blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": "*:red_circle: A-tier — work today*"}})
    ...
    if b:
        ...
        blocks.append({"type": "section", "text": {"type": "mrkdwn",
                       "text": "*:large_blue_circle: B-tier — qualified, review*\n" + "\n".join(b_lines) + tail}})
...
def main() -> int:
    ...
    path = args.file or os.path.join(args.output_dir, "web3career_jobs_%s.json" % today)
    ...
    if resp.status_code == 200:
        print("Posted brief to Slack (%d signals)." % payload.get("total_signals", 0))
        return 0
